# Source: https://www.linkedin.com/pulse/html-canvas-testing-selenium-opencv-maciej-kusz/
import cv2
import numpy
...
class GraphicalLocator(object):
    ...
    def find_me(self, drv):
        ...
        threshold = {'shape': round(img_match[1], 2), 'histogram': round(comp_hist, 2)}

        print(threshold)
        is_found = True if threshold['shape'] >= 0.85 and threshold['histogram'] >= 0.32 else False
        # Return image with blue rectangle around match
        # return cv2.rectangle(scr, (self.x, self.y), (self.x + self.width, self.y + self.height) ,(0, 0, 255), 2)
